class Friendship < ApplicationRecord
  ...
  scope :by_requester_and_responser, -> (user_id, friend_id) {
    where('user_request = ? or user_response = ? AND user_request = ? or user_response = ?', user_id, user_id, friend_id, friend_id).first
  }
